...
export default function Home() {
  const [form, setForm] = useState({ name: '', facility: '', email: '', phone: '', claims: '', license: '', message: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus('success');
        setForm({ name: '', facility: '', email: '', phone: '', claims: '', license: '', message: '' });
      } else {
  ...
  return (
    <>
      <Head>
        <title>Amro Saliem | Clinical Costing Expert - Abu Dhabi UAE</title>
        <meta name="description" content="DOH-compliant clinical costing for Abu Dhabi healthcare facilities. Just 1 AED per claim." />
      ...
      <a href="https://wa.me/971502392790" target="_blank" rel="noopener noreferrer" className="whatsapp-float" title="Chat on WhatsApp">💬</a>

      <nav>
        <div className="nav-logo">Amro Saliem</div>
      ...
      <section className="hero">
        ...
        <div className="hero-inner">
          <span className="hero-tag">🏥 DOH-Compliant Clinical Costing · Abu Dhabi UAE</span>
          <h1>Clinical Costing Made<br /><em>Simple & Affordable</em></h1>
          <p className="hero-sub">Full DOH-compliant patient-level clinical costing for hospitals, clinics, and medical centers in Abu Dhabi — delivered with precision, transparency, and the most competitive pricing in the UAE.</p>
          <div className="hero-price-block">
            <div className="price-num">1<span style={{fontSize:'1.3rem'}}>AED</span></div>
          ...
          <div className="hero-btns">
            ...
            <a href="https://wa.me/971502392790" target="_blank" rel="noopener noreferrer" className="btn-ghost">💬 WhatsApp Me</a>
          </div>
        </div>
      </section>

      <div className="badge-strip">
        {['DOH Abu Dhabi Compliant','DRG / Activity-Based Costing','Shafafiya-Ready Submissions','All Facility Types & Sizes','1 AED Per Claim'].map(b => (
          <div className="badge-item" key={b}><span className="dot" />{b}</div>
        ))}
      </div>

      <section className="why" id="why">
        <div className="inner">
          <div className="section-label">Why Choose Amro Saliem</div>
          <h2 className="section-title">Your Compliance Partner,<br />Not Just a Vendor</h2>
          <div className="why-grid">
            <div className="why-cards">
              {[
                ['🏆 UAE Market Expertise','Deep knowledge of DOH regulations, Abu Dhabi Clinical Costing Standards (Nov 2024), and Shafafiya submission requirements — built specifically for the UAE healthcare ecosystem.'],
                ['💰 Unbeatable Price — 1 AED/Claim','While competitors charge thousands per engagement, I offer per-claim pricing at just 1 AED — making compliance accessible for clinics of every size, from single-specialty to large hospitals.'],
                ['📊 DRG & ABF Alignment','Proper DRG mapping is critical under Abu Dhabi\'s value-based funding model. I structure your cost data to accurately reflect each encounter and ensure your reimbursements are never undervalued.'],
                ['✅ Support Until Accepted','I stay with you through every validation cycle. If the DOH flags an issue, I resolve it — no extra fees. You get a fully accepted, audit-ready submission, guaranteed.'],
              ].map(([title,desc]) => (
                <div className="why-card" key={title}><h4>{title}</h4><p>{desc}</p></div>
              ))}
            </div>
            <div className="why-stat-box">
              <span className="big-stat">100%</span>
              <span className="stat-caption">DOH Submission Compliance Required</span>
              <div className="divider" />
              <p>Under the Abu Dhabi Clinical Costing Road Map 2025, <strong>all healthcare facilities</strong> providing direct patient care must submit standardized, patient-level cost data — regardless of size.</p>
              <br />
              <p style={{color:'var(--teal)',fontWeight:600}}>I make sure you are never caught unprepared.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="services" id="services">
        <div className="inner">
          ...
          <h2 className="section-title">Everything You Need<br />for DOH Compliance</h2>
          <p className="section-sub">End-to-end clinical costing support — from data collection to final submission and beyond.</p>
          <div className="services-grid">
            {[
              ['🗂️','Patient-Level Cost Data Collection','Capture and organize granular financial and clinical data per encounter — direct costs (medications, physician time) and indirect costs (overhead, admin) — structured per DOH ledger standards.'],
              ['📐','Activity-Based & DRG Costing','Apply the right costing methodology for your facility. Accurate DRG mapping ensures your cost-to-tariff variance is visible and your service lines are properly funded.'],
              ['📤','DOH XML Submission Preparation','Generate fully formatted, Shafafiya-validated XML files that pass DOH technical validations on the first attempt — eliminating costly re-submissions.'],
              ['📈','Cost & Profitability Reporting','Understand your true service-line economics. Identify cost drivers, compare expected vs. actual costs per procedure, and discover where you are losing money under DRG billing.'],
              ['🔍','Audit-Ready Documentation','Maintain a complete, traceable audit trail for all submitted cost data. Should the DOH audit your facility, you will have clean, organized records ready to present instantly.'],
              ['🤝','Ongoing Compliance Support','As DOH standards evolve, I keep your process aligned. Annual resubmissions, updates to clinical costing guidelines, and new regulatory requirements — I handle it all.'],
            ].map(([icon,title,desc]) => (
      ...
      <section className="pricing" id="pricing">
        <div className="pricing-inner">
          ...
          <p className="section-sub" style={{margin:'0 auto',textAlign:'center'}}>No complex packages. No hidden fees. Just the most competitive clinical costing rate in the UAE.</p>
          <div className="pricing-card">
            <div className="ribbon">UAE Most Affordable</div>
            <div className="price-amount">1 <span style={{fontSize:'2.2rem',color:'var(--muted)'}}>AED</span></div>
            <div className="price-per">Per Claim / Patient Encounter</div>
            <ul className="price-features">
              {['Full patient-level cost data processing','DRG mapping & activity-based cost allocation','DOH-compliant XML file generation','Shafafiya validation & error correction','Audit-ready documentation package','Submission support until fully accepted','Ongoing communication & status updates','No minimum claim volume required'].map(f=><li key={f}>{f}</li>)}
            </ul>
            <a href="#contact" className="btn-primary" style={{display:'inline-flex',marginTop:'1rem'}}>📩 Start Your Submission Today</a>
            <p className="pricing-note">Volume discounts available for facilities submitting 50,000+ claims/year.<br />Free initial consultation — no commitment required.</p>
          </div>
        </div>
      </section>
      ...
      <section className="process" id="process">
        <div className="inner" style={{maxWidth:900}}>
          ...
          <h2 className="section-title">From Raw Data to<br />Accepted Submission</h2>
          <p className="section-sub">A clear, structured process designed to minimize disruption to your team.</p>
          <div className="steps">
            {[
              ['Free Consultation & Scoping','We discuss your facility type, claim volume, existing data systems (EMR/HIS/ERP), and timeline. I assess your data quality and outline exactly what is needed for your DOH submission.'],
              ['Data Collection & Mapping','I work with your finance and clinical teams to extract and structure the required financial, clinical, and operational data — mapping direct costs, indirect costs, and cost drivers per encounter.'],
              ['Costing Engine & DRG Calculation','Applying Abu Dhabi Clinical Costing Standards, I calculate patient-level costs using activity-based and DRG costing models — producing accurate, granular outputs for every encounter.'],
              ['Validation & XML Generation','I run all Shafafiya validation checks and generate DOH-compliant XML files. Any technical errors are corrected before submission — ensuring a clean first submission.'],
              ['Submission & Acceptance','I submit to the DOH portal and monitor the process until full acceptance is confirmed. You receive complete audit-ready documentation and a submission report.'],
            ].map(([title,desc],i)=>(
      ...
      <section className="about" id="about">
        <div className="about-inner">
          ...
          <h2 className="section-title">Amro Saliem<br />Clinical Costing Specialist</h2>
          <div className="about-box">
            <p>I am a qualified accountant based in Abu Dhabi with specialized expertise in healthcare finance and DOH clinical costing compliance. My background spans financial analysis, healthcare billing systems, and regulatory reporting across the UAE healthcare sector.</p>
            <p>With the rollout of the Abu Dhabi Clinical Costing Road Map 2025 mandating patient-level cost data from all direct-care facilities, I saw a clear gap: most providers were either unaware of the requirements or could not afford the high fees charged by large consulting firms.</p>
            <p>I built my practice to change that — offering the same quality of DOH-compliant clinical costing at <strong style={{color:'var(--gold)'}}>just 1 AED per claim</strong>, making compliance achievable for every facility from a small dental clinic to a multi-specialty medical center.</p>
            <div className="about-tags">
              {['DOH Clinical Costing Standard 2024','DRG / IR-DRG Mapping','Activity-Based Costing','Shafafiya Portal','Healthcare Finance','XML Submission','Abu Dhabi UAE'].map(t=>(
      ...
      <section className="contact-section" id="contact">
        <div className="contact-inner">
          ...
          <p className="section-sub">Fill in your details below and I will get back to you within 24 hours with a tailored quote for your facility.</p>
          {status==='success'?(
            <div className="success-msg">
              <h3>Enquiry Received!</h3>
              <p>Thank you! I will be in touch within 24 hours. Check your inbox — a confirmation email is on its way.</p>
            </div>
          ):(
            <form onSubmit={handleSubmit}>
              <div className="form-grid">
                <div className="form-group">
                  ...
                  <input id="name" name="name" type="text" placeholder="Dr. Ahmed Al Mansouri" value={form.name} onChange={handleChange} required />
                </div>
                <div className="form-group">
                  <label htmlFor="facility">Facility / Organisation</label>
                  <input id="facility" name="facility" type="text" placeholder="Al Noor Hospital" value={form.facility} onChange={handleChange} />
                </div>
                <div className="form-group">
                  <label htmlFor="email">Email Address *</label>
                  <input id="email" name="email" type="email" placeholder="ahmed@clinic.ae" value={form.email} onChange={handleChange} required />
                </div>
                <div className="form-group">
                  <label htmlFor="phone">Phone / WhatsApp</label>
                  <input id="phone" name="phone" type="tel" placeholder="+971 50 000 0000" value={form.phone} onChange={handleChange} />
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <select id="claims" name="claims" value={form.claims} onChange={handleChange}>
                    <option value="">Select volume...</option>
                    <option value="Under 1,000">Under 1,000 claims</option>
                    <option value="1,000 - 5,000">1,000 - 5,000 claims</option>
                    <option value="5,000 - 20,000">5,000 - 20,000 claims</option>
                    <option value="20,000 - 50,000">20,000 - 50,000 claims</option>
                    <option value="50,000+">50,000+ claims</option>
                  </select>
                </div>
                <div className="form-group full">
                  <label htmlFor="message">Message / Questions *</label>
                  <textarea id="message" name="message" placeholder="Tell me about your facility, your current costing setup, and any specific challenges you are facing with DOH submission..." value={form.message} onChange={handleChange} required />
                </div>
              </div>
              {status==='error'&&<div className="error-msg">⚠️ {errorMsg}</div>}
              <button type="submit" className="btn-primary form-submit" disabled={status==='loading'}>
                {status==='loading'?'⏳ Sending...':'📩 Send My Enquiry — Free Consultation'}
              </button>
              <p style={{fontSize:'0.78rem',color:'var(--muted)',textAlign:'center',marginTop:'1rem'}}>No spam. No commitment. I typically respond within a few hours.</p>
            </form>
          )}
          <div style={{display:'flex',justifyContent:'center',gap:'2rem',flexWrap:'wrap',marginTop:'2.5rem'}}>
            <div style={{display:'flex',alignItems:'center',gap:'0.6rem',fontSize:'0.87rem',color:'var(--muted)'}}>
              <span>📍</span><span>Abu Dhabi, UAE</span>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:'0.6rem',fontSize:'0.87rem',color:'var(--muted)'}}>
              <span>💬</span>
              <a href="https://wa.me/971502392790" target="_blank" rel="noopener noreferrer" style={{color:'var(--teal)',textDecoration:'none',fontWeight:600}}>WhatsApp: +971 50 239 2790</a>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:'0.6rem',fontSize:'0.87rem',color:'var(--muted)'}}>
              <span>🕐</span><span>Sun–Thu, 8am–6pm</span>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:'0.6rem',fontSize:'0.87rem',color:'var(--muted)'}}>
              <span>✉️</span><span>Free Initial Consultation</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer>
        <p style={{fontFamily:'Playfair Display,serif',fontSize:'1rem',color:'var(--white)',marginBottom:'0.4rem'}}>Amro Saliem — Clinical Costing Specialist</p>
        <p>Abu Dhabi, United Arab Emirates · DOH-Compliant Clinical Costing · 1 AED Per Claim</p>
      </footer>
    </>
  );
}
